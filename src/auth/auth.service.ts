...
export class AuthService{
    ...
    async removeTokens(tokensDto: TokensDto){
        await this.userService.addingTokensToBlacklist(tokensDto);
    }

    async isAccessTokenInBlacklist(token: string): Promise<boolean> {
        return (await this.userService.accessTokenInBlacklist(token));
    }

    async isRefreshTokenInBlacklist(token: string): Promise<boolean> {
